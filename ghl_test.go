...
func TestGHL_GetDownloadURL(t *testing.T) {
	...
	if got, err := g.GetDownloadURL("hatyuki/go-ghl@foo", "", ""); err == nil {
		t.Error("should return an error")
	} else if got != "" {
		t.Errorf("returns wrong URL: got %v want %v", got, "")
	}

	if got, err := g.GetDownloadURL("hatyuki/go-ghl", "", ""); err != nil {
	...
	os := "linux"
	arch := "386"
	if got, err := g.GetDownloadURL("hatyuki/go-ghl", os, arch); err != nil {
		t.Errorf("returns an error: got %v", err)
	} else if got == "" {
		t.Error("should return URL")
	...
	if got, err := g.GetDownloadURL("hatyuki/go-ghl@v0.0.1", "", ""); err != nil {
		t.Errorf("returns an error: got %v", err)
	} else if got == "" {
		t.Error("should return URL")
	} else if tag := "v0.0.1"; !strings.Contains(got, tag) {
		t.Errorf("returns wrong URL: got %v", got)
	}
}
